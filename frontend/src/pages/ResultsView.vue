...
async function fetchWebResults(query) {
  try {
    const res = await fetch(`${API_BASE}/api?q=${encodeURIComponent(query)}`)
...
async function performSearch(query) {
  ...
  results.value = filterByPrism(webFiltered)
    ...
    <main class="content-area">
      ...
      <template v-else-if="hasSearched">
        <div v-if="activeTab === 'all'" class="all-content">
          ...
          <div v-if="results.length === 0 && !weatherData && !showCalculator" class="empty-state">
    ...
    <span v-if="getPath(item.data.url)" class="result-path">{{ getPath(item.data.url) }}</span>
        ...
        <div v-else-if="activeTab === 'news'" class="news-content">
          <div v-if="newsResults.length === 0" class="empty-state">
            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
              <path d="M4 22h16a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2H8a2 2 0 0 0-2 2v16a2 2 0 0 1-2 2Zm0 0a2 2 0 0 1-2-2v-9c0-1.1.9-2 2-2h2"/><line x1="10" y1="6" x2="18" y2="6"/><line x1="10" y1="10" x2="18" y2="10"/>
            </svg>
            <p class="empty-query">{{ t.results.noResults }} "{{ searchQuery }}"</p>
            <p class="empty-hint">{{ t.results.tryDifferent }}</p>
          </div>
...
@media (max-width: 768px) {
  ...
  .prism-pill { padding: 4px 10px; font-size: 0.74rem; }
  .prism-pill-label { font-size: 0.74rem; }
}
...
@media (max-width: 480px) {
  ...
  .result-header { gap: 5px; margin-bottom: 1px; min-width: 0; justify-content: flex-start; width: 100%; padding-right: 32px; }
  .result-favicon { width: 16px; height: 16px; }
  ...
  .weather-location { font-size: 0.76rem; }
  ...
  .calc-history-expr { font-size: 0.76rem; }
  .calc-history-result { font-size: 0.8rem; }
  ...
  .preview-source-btn { padding: 8px 12px; font-size: 0.76rem; min-height: 40px; }
  ...
  .try-again-btn { padding: 10px 20px; min-height: 44px; font-size: 0.84rem; }
...
@media (hover: none) and (pointer: coarse) {
  ...
  .prism-pill { -webkit-tap-highlight-color: transparent; }
  .prism-pill:active { transform: scale(0.95); }
}
</style>