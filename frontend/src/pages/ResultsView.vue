...
import { initSettings, highlightTerms } from '../composables/settings.js'
...
function highlightedDescription(text) {
  if (!text) return ''
  if (!highlightEnabled.value || !searchQuery.value) return escapeHtml(text)
  return highlightTerms(escapeHtml(text), searchQuery.value)
}

function highlightedTitle(text) {
  if (!text) return ''
  const trimmed = text.split(' ').slice(0, 35).join(' ') + (text.split(' ').length > 35 ? '...' : '')
  if (!highlightEnabled.value || !searchQuery.value) return escapeHtml(trimmed)
  return highlightTerms(escapeHtml(trimmed), searchQuery.value)
}
...
onMounted(async () => {
  ...
  highlightEnabled.value = localStorage.getItem('synapic_highlight') !== '0'
              ...
              <div class="results-list">
                <template v-for="(item, idx) in paginatedResults" :key="item.type === 'result' ? 'r-' + item.index : item.type + '-' + idx">
                  <article v-if="item.type === 'result'" class="result-card">
                    ...
                    <a :href="item.data.url" target="_blank" rel="noopener noreferrer" class="result-title" v-html="highlightedTitle(item.data.title)"></a>
                    <p v-if="item.data.description" class="result-desc" v-html="highlightedDescription(item.data.description)"></p>
