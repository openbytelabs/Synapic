...
const fetchAIAnswer = async (query) => {
  ...
  try {
    ...
    // /ai endpoint'ini kullanarak AI cevabı al
    const aiUrl = `${AI_ENDPOINT}?q=${encodeURIComponent(query)}&apikey=`;
    ...
    if (data.answer) {
      console.log('[AI] Successfully got answer from AI endpoint');
      aiAnswer.value = data.answer;
    } else {
      console.log('[AI] No answer in response');
